<footer class="bg-primary text-light py-4">
    <div class="container">
        <div class="row">
            <!-- Company Information -->
            <div class="col-md-4 mb-3">
                <h5>About Software Farm</h5>
                <p>
                    Software Farm (Pty) Ltd specializes in providing cutting-edge financial management software for farmers, helping them streamline operations and maximize productivity.
                </p>
            </div>

            <!-- Quick Links -->
            <div class="col-md-4 mb-3">
                <h5>Quick Links</h5>
                <ul class="list-unstyled">
                    <li><a href="index.php" class="text-light text-decoration-none">Home</a></li>
                    <li><a href="about.php" class="text-light text-decoration-none">About Us</a></li>
                    <li><a href="products.php" class="text-light text-decoration-none">Products</a></li>
                    <li><a href="training.php" class="text-light text-decoration-none">Training</a></li>
                </ul>
            </div>

            <!-- Contact Information -->
            <div class="col-md-4 mb-3">
                <h5>Contact Us</h5>
                <p>
                    <i class="bi bi-telephone me-2"></i> 
                    <a href="tel:+27123652683" class="text-light text-decoration-none">012 365 2683</a>
                </p>
                <p>
                    <i class="bi bi-envelope me-2"></i> 
                    <a href="mailto:support@softwarefarm.co.za" class="text-light text-decoration-none">support@softwarefarm.co.za</a>
                </p>
                <p>
                    <i class="bi bi-geo-alt me-2"></i> 
                    18 Sawgrass Ave, Silver Lakes Golf Estate, 0081
                </p>
            </div>
        </div>

        <div class="text-center mt-3">
            <p class="mb-0">&copy; <?php echo date("Y"); ?> Software Farm (Pty) Ltd. All rights reserved.</p>
        </div>
    </div>
</footer>
